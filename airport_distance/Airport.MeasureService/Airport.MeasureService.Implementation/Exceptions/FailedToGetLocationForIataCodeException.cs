namespace Airport.MeasureService.Implementation.Exceptions;

/// <summary>
/// Exception thrown when unable to retrieve location data for a given IATA code.
/// </summary>
public class FailedToGetLocationForIataCodeException : Exception
{
    /// <summary>
    /// Initializes a new instance of the <see cref="FailedToGetLocationForIataCodeException"/> class.
    /// </summary>
    public FailedToGetLocationForIataCodeException()
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="FailedToGetLocationForIataCodeException"/> class with a specified error message.
    /// </summary>
    /// <param name="message">The message that describes the error.</param>
    public FailedToGetLocationForIataCodeException(string? message) : base(message)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="FailedToGetLocationForIataCodeException"/> class with a specified error message
    /// and a reference to the inner exception that is the cause of this exception.
    /// </summary>
    /// <param name="message">The error message that explains the reason for the exception.</param>
    /// <param name="innerException">The exception that is the cause of the current exception.</param>
    public FailedToGetLocationForIataCodeException(string? message, Exception? innerException) : base(message, innerException)
    {
    }
}